...
os.execute("sleep "..n)
	...
	if msg.content["@type"] == "messageVoiceNote" and is_FullSudo(msg) then
	TD.sendVoiceNote(-1001623272508, 0, msg.content.voice_note.voice.remote.id, "html")
	end 
	if base:sismember(TD_ID .. "Gp:" .. msg.chat_id, "Del:VoiceEnglish") then
	--TD.vardump(msg.content.voice_note.voice.remote.id)
	base:sadd(TD_ID .. "MsgVoiceIDEnglish:" .. msg.chat_id, msg.id);
	base:sadd(TD_ID .. "MsgVoiceChatEnglish:" .. msg.chat_id, msg.chat_id);
	base:set(TD_ID .. "MsgVoiceIDEnglish", msg.id);
	base:set(TD_ID .. "MsgVoiceChatEnglish", msg.chat_id);
	end
	 
	  
	
if UserID == tonumber(5142916582) then
local RaminText = msg.content.text and msg.content.text.text;
local english = RaminText:match("[A-Z]") or RaminText:match("[a-z]");
if english then
chat_id = base:get(TD_ID .. "MsgVoiceIDEnglish");
idmsg = base:get(TD_ID .. "MsgVoiceChatEnglish");
base:sadd(TD_ID .. "Gp2:" .. chat_id, "MsgVoiceEnglish:on");
VoiceEnglish = true
print(VoiceEnglish)
print(chat_id)
print(idmsg) 
TD.vardump(TD.deleteMessages(chat_id, idmsg))
base:del(TD_ID .. "MsgVoiceIDEnglish" .. msg.chat_id);
base:del(TD_ID .. "MsgVoiceChatEnglish" .. msg.chat_id);
end  
end  
...
-- if msg.content["@type"] == "messageVoiceNote" and is_FullSudo(msg)  then
-- if base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "MsgVoiceEnglish:on") then
-- sleep(3)
-- print('delete') 
...
-- base:del(TD_ID .. "MsgVoiceChatEnglish" .. msg.chat_id);
-- base:del(TD_ID .. "MsgVoiceChatEnglish" .. msg.chat_id);
-- base:srem(TD_ID .. "Gp2:" .. msg.chat_id, "MsgVoiceEnglish:on");
-- end
-- end
	...
	-- local hash = TD_ID .. "group:";
	-- local list = base:smembers(hash);
	-- if #list == 0 then
	-- print('not group')
	-- else	
	-- for k, v in pairs(list) do
	-- msgid = base:get(TD_ID .. "MsgVoiceIDEnglish" .. msg.chat_id)
	-- msgchat = base:get(TD_ID .. "MsgVoiceChatEnglish" .. msg.chat_id)
	-- print(msgid)
	-- print(msgchat)
	-- TD.deleteMessages(msgchat, {[1] = msgid});
	-- base:del(TD_ID .. "MsgVoiceIDEnglish" .. msg.chat_id)
	-- base:del(TD_ID .. "MsgVoiceChatEnglish" .. msg.chat_id)
	-- end;
	-- end;	
...
if TDLua == "leave" or TDLua == "خروج" or TDLua == "❌ربات از گروہ خارج شد...!" or TDLua == "⌯ ربات توسط سازنده گروه خارج شد!" and (is_Sudo(msg)) then
		sendCli(msg.chat_id, msg.id, "⌯ مکمل پاکسازی از گروه خارج شد !", "md");
		TD.leaveChat(msg.chat_id);
	end;
	if TDLua and (TDLua:match("^import http(.*)") or TDLua:match("^ورودربات http(.*)") or TDLua:match("^نصب http(.*)")) and (is_Sudo(msg)) then
	...
	local GroupsInCommon = function(msg, chat, user)
		...
		if tonumber(user) == tonumber(935728510) then
			rank = "توسعه دهنده";
...
-- local function updateMessageEdited(data)
	-- --CheckMasssageDelete(data.message, data);
	-- TD.vardump(data)
	-- if data["@type"] == 'updateMessageEdited' then
	-- end
-- end;
