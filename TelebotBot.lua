...
local MrTeleGrami = function(msg, data)
	...
	if base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "added") then
		...
		local lang = base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "telebotlang");
		local reportpv = base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "reportpv");
		local ownerslist = base:smembers(TD_ID .. "OwnerList:" .. msg.chat_id);
		local reportpv = base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "reportpv");
		local reportpvall = base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "reportpvall");
...
base:setex(TD_ID .. "teletab:" .. msg.chat_id, 3600, true);
	...
	if tonumber(msg.sender_id.user_id) == tonumber(5142916582) then
	TD.vardump(msg)
	local RaminText = msg.content.text and msg.content.text.text;
	end
		...
		if base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "added") and ChatTypeSuperGp then
			...
			if msg.content["@type"] == "messageDocument" and not is_Vip(msg) then
			if base:sismember(TD_ID .. "Gp:" .. msg.chat_id, "Del:Android") then
			RaminFile = msg.content.document.file_name
			if msg.content.document["@type"] == 'document' and msg.content.document.mime_type == 'application/vnd.android.package-archive' then
			local filesexy = RaminFile:find("apk")
			if filesexy then
            lock_del(msg, "ارسال# فایل اندروید");
			end
			end
			end
			end
...
if msg.sender_id.user_id and is_GlobalyMute(msg.sender_id.user_id) and not is_Vip(msg) then		
	local usere= msg.sender_id.user_id
		...
		if base:sismember(TD_ID .. "Gp2:" .. cht, "added") then
			...
			if Ramin then
				...
				if TDMatches and is_JoinChannel(msg) and is_ModLock(msg.chat_id, msg.sender_id.user_id) then
					local returntd1 = TDMatches:match("^photo$") or TDMatches:match("^fileandroid$") or TDMatches:match("^game$") or TDMatches:match("^video$") or TDMatches:match("^flood$") or TDMatches:match("^inline$") or TDMatches:match("^videomsg$") or TDMatches:match("^caption$") or TDMatches:match("^voice$") or TDMatches:match("^location$") or TDMatches:match("^document$") or TDMatches:match("^contact$") or TDMatches:match("^text$") or TDMatches:match("^sticker$") or TDMatches:match("^stickervideo$") or TDMatches:match("^stickers$") or TDMatches:match("^gif$") or TDMatches:match("^music$") or TDMatches:match("^عکس$") or TDMatches:match("^بازی$") or TDMatches:match("^فایل اندروید$") or TDMatches:match("^فیلم$") or TDMatches:match("^اینلاین$") or TDMatches:match("^دکمه شیشه ای$") or TDMatches:match("^ویدیومسیج$") or TDMatches:match("^فیلم سلفی$") or TDMatches:match("^کپشن$") or TDMatches:match("^موقعیت$") or TDMatches:match("^لوکیشن$") or TDMatches:match("^ویس$") or TDMatches:match("^صدا$") or TDMatches:match("^فایل$") or TDMatches:match("^مخاطب$") or TDMatches:match("^متن$") or TDMatches:match("^استیکر$") or TDMatches:match("^استیکر ویدئو$") or TDMatches:match("^استیکر ویدیو$") or TDMatches:match("^استیکر متحرک$") or TDMatches:match("^گیف$") or TDMatches:match("^اهنگ$") or TDMatches:match("^آهنگ$") or TDMatches:match("^موزیک$") or TDMatches:match("^spam$") or TDMatches:match("^اسپم$") or TDMatches:match("^فلود$") or TDMatches:match("^رگباری$") or TDMatches:match("^پست$") or TDMatches:match("^channelpost$") or TDMatches:match("^بدافزار$") or TDMatches:match("^malware$");
					...
					local function tdlock(Guard)
						...
						elseif Guard:match("^game$") or Guard:match("^بازی$") then
							...
							tde = "gαмε";
							tdf = "بازی";
							elseif Guard:match("^fileandroid$") or Guard:match("^فایل اندروید$") then
							td = "Android";
							tde = "Android";
							tdf = "فایل اندروید";
			...
			if is_Sudo(msg) then
				...
				if Ramin == "reload" or Ramin == "بروز" and is_Sudo(msg) then
					...
					for k, v in pairs({
						...
						os.execute("rm -rf ~/TeleBot/.tdlua-sessions/APIBot/" .. v .. "/*");
						...
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/db.sqlite-wal");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/db.sqlite");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/photos/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/animations/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/videos/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/music/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/voice/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/temp/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/documents/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/video_notes/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/stickers/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/thumbnails/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/APIBot/profile_photos/*");
						os.execute("rm -rf ~/TeleBot/.tdlua-sessions/TabchiBot/" .. v .. "/*");
						io.popen("sync && echo 3 > /proc/sys/vm/drop_caches");
						io.popen("swapoff -a && swapon -a");
						io.popen("rm -rf $PWD/.tdlua.log");
						io.popen("rm -rf $PWD/.tdlua.log.old");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/db.sqlite-wal");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/db.sqlite");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/photos/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/animations/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/videos/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/music/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/voice/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/temp/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/documents/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/video_notes/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/stickers/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/thumbnails/*");
						io.popen("rm -rf $PWD/.tdlua-sessions/TabchiBot/profile_photos/*");
...
local text100 = Ramin:match("^هواشناسی (.*)$")

local url , res = http.request('https://api.openweathermap.org/data/2.5/weather?q='..text100..'&APPID=1312815a8d5a3338c39e1c671dbd4315&units=metric')
if res ~= 200 then
return
end
local jdat = json:decode(url)
tol = os.date("%X", jdat.sys.sunrise) 
gor= os.date("%X", jdat.sys.sunset)
local url , res = http.request('http://api.openweathermap.org/data/2.5/air_pollution?lat='..jdat.coord.lat..'&lon='..jdat.coord.lon..'&appid=1312815a8d5a3338c39e1c671dbd4315')
if res ~= 200 then
return
end
local jdatpoor = json:decode(url)
if jdatpoor.list[1].main.aqi == 1 then
poor = "خوب"
elseif jdatpoor.list[1].main.aqi == 2 then
poor = "در حد متوسط"
elseif jdatpoor.list[1].main.aqi == 3 then
poor = "ناسالم برای گروه های حساس"
elseif jdatpoor.list[1].main.aqi == 4 then
poor = "ناسالم"
elseif jdatpoor.list[1].main.aqi == 5 then
poor = "خیلی ناسالم"
end

if jdatpoor.list[1].components.no2 < 40 then
no2 = "خوب"
elseif jdatpoor.list[1].components.no2 > 41 then
no2 = "در حد متوسط"
elseif jdatpoor.list[1].components.no2 > 181 then
no2 = "خیلی زیاد"
elseif jdatpoor.list[1].components.no2 > 281 then
no2 = "خطرناک"
end

coo = math.floor(jdatpoor.list[1].components.co / 1000)
if coo < math.floor(1.1) then
co = "خوب"
elseif coo > math.floor(2.1) then
co = "در حد متوسط"
elseif coo > math.floor(10) then
co = "خیلی زیاد"
elseif coo > math.floor(17) then
co = "خطرناک"
end
if jdatpoor.list[1].components.o3 < 50 then
o3 = "خوب"
elseif jdatpoor.list[1].components.o3 > 51 then
o3 = "در حد متوسط"
elseif jdatpoor.list[1].components.o3 > 101 then
o3 = "خیلی زیاد"
elseif jdatpoor.list[1].components.o3 > 169 then
o3 = "خطرناک"
end
if jdatpoor.list[1].components.so2 < 40 then
so2 = "خوب"
elseif jdatpoor.list[1].components.so2 > 41 then
so2 = "در حد متوسط"
elseif jdatpoor.list[1].components.so2 > 81 then
so2 = "خیلی زیاد"
elseif jdatpoor.list[1].components.so2 > 381 then
so2 = "خطرناک"
end

if jdatpoor.list[1].components.pm10 < 50 then
pm10 = "خوب"
elseif jdatpoor.list[1].components.pm10 > 51 then
pm10 = "در حد متوسط"
elseif jdatpoor.list[1].components.pm10 > 101 then
pm10 = "خیلی زیاد"
elseif jdatpoor.list[1].components.pm10 > 251 then
pm10 = "خطرناک"
end
nh31 = math.floor(jdatpoor.list[1].components.nh3 * 100)
if nh31 < math.floor(200) then
nh3 = "خوب"
elseif nh31 > math.floor(201) then
nh3 = "در حد متوسط"
elseif nh31 > math.floor(401) then
nh3 = "خیلی زیاد"
elseif nh31 > math.floor(801) then
nh3 = "خطرناک"
end
text = "*✦ آب و هوای شهر "..text100.." به شرح ذیل می باشد !*\n\n┅┅━آب و هوا━┅┅\n\n✦ میزان درجه هوا : "..jdat.main.temp.." °С*\n*✦کمترین درجه : "..jdat.main.temp_min.." °С*\n*✦بالاترین درجه : "..jdat.main.temp_max.." °С*\n*✦درصد مرطوب : "..jdat.main.humidity.."%*\n*✦درجه باد بر مایل : "..jdat.wind.speed.." m/s*\n*✦ابر : "..jdat.clouds.all.." hpa*\n\n• ┅┅━وضعیت آلودگی━┅┅ •\n\n✦ مونو اکسید کربن : *"..co.."*\n✦ دی اکسید نیتروژن : *"..no2.."*\n✦ لایه اوزون : *"..o3.."*\n✦ دی اکسید گوگرد: *"..so2.."* \n✦ ریز گرد ها : *"..pm10.."*\n✦ آمونیاک : *"..nh3.."*\n✦ نتیجه گیری :\n✦کیفیت هوای امروز "..text100.." *"..poor.." می باشد !\n\n🆔 *"..Channel.."*"
--sendText(msg.chat_id,text,msg.message_id, 'md')

sendBot(msg.chat_id, msg.id,text,"md")  
end
if Ramin and (Ramin:match('^اذان (.*)'))  then
local text = Ramin:match('اذان (.*)')
local url , res = http.request('https://api.codebazan.ir/owghat/?city='..text..'')
if res ~= 200 then
return
end 
local jdat = json:decode(url)
shahr = jdat.Result[1].shahr
tarikh = jdat.Result[1].tarikh
azansobh = jdat.Result[1].azansobh
toloaftab = jdat.Result[1].toloaftab
azanzohr = jdat.Result[1].azanzohr
ghorubaftab = jdat.Result[1].ghorubaftab
azanmaghreb = jdat.Result[1].azanmaghreb
nimeshab = jdat.Result[1].nimeshab
local azan = '🕌 اوقات شرعی امروز به صورت ذیل است :\n┅┅━اوقات شرعی━┅┅\n✦ نام شهر : '..shahr..'\n✦ تاریخ امروز : '..tarikh..'\n✦ اذان صبح : '..azansobh..'\n✦ طلوع آفتاب : '..toloaftab..'\n✦ اذان ظهر : '..azanzohr..'\n✦ غروب آفتاب : '..ghorubaftab..'\n✦ اذان مغرب : '..azanmaghreb..'\n✦ نیمه شب شرعی : '..nimeshab..'\n\n🆔 *'..Channel..'*'
sendBot(msg.chat_id, msg.id,azan,"md")
		...
		if Ramin and (Ramin:match("^زمان ورود$") or Ramin and Ramin:match("^jointime$")) and tonumber(msg.reply_to_message_id) > 0 and is_ModVipCmd(msg.chat_id, msg.sender_id.user_id) and is_JoinChannel(msg) then
			res = TD.getMessage(msg.chat_id, tonumber(msg.reply_to_message_id));
			if res.content["@type"] == "messageDocument" or res.content["@type"] == "messageAudio" or res.content["@type"] == "messageVoiceNote" or res.content["@type"] == "messageSticker" or res.content["@type"] == "messageAnimation" or res.content["@type"] == "messagePhoto" or res.content["@type"] == "messageVideoNote" then
				dofile("./checkuser.lua");
				UserJoin(msg, msg.chat_id, res.sender_id.user_id);
			end;
		end;
		if Ramin and (Ramin:match("^زمان ورود$") or Ramin and Ramin:match("^jointime$")) and tonumber(msg.reply_to_message_id) > 0 and is_ModVipCmd(msg.chat_id, msg.sender_id.user_id) and is_JoinChannel(msg) then
			res = TD.getMessage(msg.chat_id, tonumber(msg.reply_to_message_id));
			...
			if text:match("^@(.*)$") then
				...
				if result.id then
					dofile("./checkuser.lua");
					UserJoin(msg, msg.chat_id, result.id);
				else
					sendBot(msg.chat_id, msg.id, "⌯  کاربر  ▏  @" .. username .. " ▕ یافت نشد !", "md");
				end;
			elseif text:match("^(%d+)$") then
				local id = text:match("^(%d+)$");
				dofile("./checkuser.lua");
				UserJoin(msg, msg.chat_id, tonumber(id));
			elseif res.content.text.entities[1] and res.content.text.entities[1].type["@type"] == "textEntityTypeMentionName" then
				Ramin = TD.getUser(res.content.text.entities[1].type.user_id);
				if Ramin.id then
					dofile("./checkuser.lua");
					UserJoin(msg, msg.chat_id, Ramin.id);
				end;
			else
				result = TD.getMessage(msg.chat_id, tonumber(msg.reply_to_message_id));
				dofile("./checkuser.lua");
				UserJoin(msg, msg.chat_id, result.sender_id.user_id);
			end;
		end;
		if RaminEnti and Ramin and (Ramin:match("^زمان ورود (.*)$") or Ramin and Ramin:match("^jointime (.*)$")) and msg.content.text.entities and msg.content.text.entities[1] and msg.content.text.entities[1].type["@type"] and tonumber(msg.reply_to_message_id) == 0 and is_ModVipCmd(msg.chat_id, msg.sender_id.user_id) and is_JoinChannel(msg) then
			id = msg.content.text.entities[1].type.user_id;
			dofile("./checkuser.lua");
			UserJoin(msg, msg.chat_id, id);
		end;
		if Ramin and (Ramin:match("^زمان ورود @(.*)$") or Ramin and Ramin:match("^jointime @(.*)$")) and tonumber(msg.reply_to_message_id) == 0 and is_ModVipCmd(msg.chat_id, msg.sender_id.user_id) and is_JoinChannel(msg) then
			local username = Ramin:match("^زمان ورود @(.*)$") or Ramin:match("^jointime @(.*)$")
			result = TD.searchPublicChat(username);
			if result.id then
				dofile("./checkuser.lua");
				UserJoin(msg, msg.chat_id, result.id);
		...
		if Ramin and (Ramin:match("^زمان ورود (%d+)$") or Ramin and Ramin:match("^jointime (%d+)$")) and tonumber(msg.reply_to_message_id) == 0 and is_ModVipCmd(msg.chat_id, msg.sender_id.user_id) and is_JoinChannel(msg) then
			dofile("./checkuser.lua");
			UserJoin(msg, msg.chat_id, Ramin:match("^زمان ورود (%d+)$") or Ramin:match("^jointime (%d+)$"));
		end;
		...
		UnRes(msg.chat_id, id);
		base:srem("AGTMute:", id);
		base:sadd("AGTMuteRem:", id);
		base:del("AGTMuteMode:",id);
				--sendBot(msg.chat_id, msg.id, " ⌯  کاربر  [ " .. id .. " ] یافت نشد !", "md");
				sendBot(msg.chat_id, msg.id, "☑️ کاربر 〚 " .. id .. "〛 از لیست تبچی ها حذف شد!\n\n  توسط : <a href=\"tg://user?id=" .. msg.sender_id.user_id .. "\"> " .. msg.sender_id.user_id .. "</a> ","html");
			end;
		end;
...
TD.sendPhoto(msg.chat_id, msg.id, "/home/ramin/new/tagvim/"..m..".png",text, "html");
end;
...
local MrTeleGramiChatMember = function(msg, data)
	...
	if ChatTypeSuperGp and not ChatTypeChannel and not ChatTypePV then
		...
		if msg.new_chat_member.status._ == "chatMemberStatusBanned" and msg.old_chat_member.status._ == "chatMemberStatusMember" then
			Userban = true;
			base:del(TD_ID .. "UserJoin" .. ChatID .. UserID);
		...
		if msg.new_chat_member.status._ == "chatMemberStatusMember" and msg.old_chat_member.status._ == "chatMemberStatusLeft" then
			if UserID == UserCheck then
				Usernewjoin = true;
				if base:get(TD_ID .. "UserJoin" .. ChatID .. UserCheck) then
				print('UserJoin OK')
				else
                salen = jdate("#Y");
				mahen = jdate("#M");
				...
				local TarikhEN = "★ روز " .. roz .. " " .. tarikhnumberen .. "\n★ ساعت : " .. timeen .. "";
				base:set(TD_ID .. "UserJoin" .. msg.chat_id .. UserCheck, TarikhEN);
				end
		...
		if msg.new_chat_member.status._ == "chatMemberStatusLeft" and msg.old_chat_member.status._ == "chatMemberStatusAdministrator" then
			UserleftAdmin = true;
			base:del(TD_ID .. "UserJoin" .. msg.chat_id .. UserID);
		...
		if Usernewjoin or Usernewadd and not Useraddban  then
			UserID = msg.actor_user_id;
			ChatID = msg.chat_id;
			print(Usernewjoin)
			print(Usernewadd)
	...
	if UserMute or Userban and not tonumber(msg.actor_user_id) == tonumber(BotJoiner) and not tonumber(msg.actor_user_id) == tonumber(BotCliId) and (not is_FullChat(msg)) and (not is_SudoChat(msg)) and (not is_OwnerChat(msg)) then
		print("remove user");
		if base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "Kheyanat:on") then
					...
					do
						do
							for k, i in pairs(base:smembers(TD_ID .. "_Sender_user_ids:" .. msg.chat_id)) do
								...
								if tonumber(getUser) == tonumber(msgs[(#msgs)]) then
_resultTEXT = _resultTEXT .. "- مقام <b>" ..Alpha(rank).."</b> :\n <b>(" .. Alpha(getUser) .. " پیام | " .. MentionUser(i) .. ")</b>\n\n";
									table.remove(msgs, #msgs);
			...
			counter = tonumber(base:get(TD_ID .. "ST:Number:" .. msg.chat_id)) or 10;
			if counter then
