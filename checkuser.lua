...
function UserJoin(msg, chat, user)
	...
	if base:get(TD_ID .. "UserJoin" .. chat .. user) then
	t = base:get(TD_ID .. "UserJoin" .. chat .. user)
		sendBot(chat, msg.id,"✅ کاربر 〚 " .. ID .. " 〛:\n"..t, "html");
	else
		sendBot(chat, msg.id, "زمان ورودش ثبت نشده است !", "html");
	end;
end;
...
function PrivateMemeber(msg, chat, user)
 ...
 function Statususer(msg, chat, user)
	...
	tab = "" .. (base:get("AGTMuteMode:"..user) or "نامعلوم ❌") .. "";
...
function untabchi(msg, chat, user)
	...
	if not base:sismember("AGTMute:", user) then
		...
		UnRes(chat, user);
		base:srem("AGTMute:", user);
		base:sadd("AGTMuteRem:", user);
		base:del("AGTMuteMode:",user);
	end;
end;
